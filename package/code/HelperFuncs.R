...
myggscatter = function(x, y, col = NULL, col.name = "color", xlab = "", ylab = "", label = NULL, line = T, num.label = 20, label.select=NULL, ...){
	...
	colnames(df) = c("x","y")
	if(!is.null(label.select) && length(label.select) > 40) {
		label.select = sample(label.select,40)
		warning("More than 40 labels were subsampled to 40")
	}
	if(!is.null(col)) {
		df$color = col
...
getPrediction = function (data, y, nrep = 100) {
	...
	return(list("predictionRes" = prediction, "averagedCoef" = coef))
}
...
predictScatter = function(real, predict, ylim = NULL, xlim = NULL, title = NULL, xlab = NULL, ylab=NULL,...) {
	...
	p = ggscatter(data, "real", "predict",
				  ...
				  cor.coeff.args = list(method = "pearson", label.x = -xlim, label.y = 0.8*ylim, label.sep = "\n", size=3),
				  title = title,
				  xlab = xlab,
				  ylab = ylab)
	p
...
mypheatmap = function(data, sign = NULL, ...) {
	
	rr = max(abs(data), na.rm = T)
